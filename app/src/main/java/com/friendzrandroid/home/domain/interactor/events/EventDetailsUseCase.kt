...
class EventDetailsUseCase@Inject constructor(val repo: EventRepository) :
    BaseUseCase<String, ResultWrapper<BaseDataWrapper<EventDetails>>>() {

    override fun execute(params: String): Flow<ResultWrapper<BaseDataWrapper<EventDetails>>> {
        return flow {
            emit(repo.getEventsDetails(params))
        }
    }
