...
class GetInterestsUseCase@Inject constructor(val repo:EventRepository):BaseUseCase<Any,ResultWrapper<BaseDataWrapper<List<InterestData>>>>() {

    override fun execute(params: Any): Flow<ResultWrapper<BaseDataWrapper<List<InterestData>>>> {
        return flow {
            emit(repo.getInterests())
        }
    }
}