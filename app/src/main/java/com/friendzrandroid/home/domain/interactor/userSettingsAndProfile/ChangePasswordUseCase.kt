...
class ChangePasswordUseCase@Inject constructor(val repo:AccountRepository):BaseUseCase<ChangePasswordRequest,ResultWrapper<BaseDataWrapper<Any>>>() {
    override fun execute(params: ChangePasswordRequest): Flow<ResultWrapper<BaseDataWrapper<Any>>> {
        return flow {
            emit(repo.changePassword(params.oldPassword,params.newPassword))
        }
    }
}