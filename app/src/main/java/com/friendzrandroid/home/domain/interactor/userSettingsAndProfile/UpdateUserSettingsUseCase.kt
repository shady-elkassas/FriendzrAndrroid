...
class UpdateUserSettingsUseCase@Inject constructor(val repo:AccountRepository):BaseUseCase<UserSettingsRequest,ResultWrapper<BaseDataWrapper<UserSettingsResponse>>>() {
    override fun execute(params: UserSettingsRequest): Flow<ResultWrapper<BaseDataWrapper<UserSettingsResponse>>> {
        return flow {
            emit(repo.updateUserSettings(params))
        }
    }
}